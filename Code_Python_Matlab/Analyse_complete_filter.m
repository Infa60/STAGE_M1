...
IndivForConvert = dir(fullfile(origineC3D, 'Participant*'));
...
%% Récupération du nombre de fichier à traiter pour chaque individu
    ...
    for trial= 1:nTrials
      ...
      % Paramètres de filtrage pour les marqueurs
        mFc = 15;
        mFs = 100;
        
        % Paramètres de filtrage pour les forces
        fFc = 15;
        fFs = 2000;
        
        % Filtre les données de marqueurs
        fc = mFc;
        fs = mFs;
        ...
        % Convertir les données filtrées en tables OpenSim
        markerTableNew = osimTableFromStruct(markerStruct);
        ...
        %% Ecriture des marker and force data to file
        ...
        %c3d.writeTRC(exportPath,markerTable);
        TRCFileAdapter().write( markerTableNew, exportPath)
        disp(['Marker file written to ' exportPath]);
